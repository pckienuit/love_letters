class FallingTextEffect {
    ...
    drawLetter(letter) {
        ...
        // Kiểm tra projected.scale hợp lệ
        if (!projected.scale || projected.scale <= 0) {
            return;
        }

        this.ctx.save();
        
        // Hiệu ứng glow với scale
        this.ctx.shadowColor = letter.color;
        this.ctx.shadowBlur = Math.max(0, 4 * projected.scale);
        this.ctx.fillStyle = letter.color;
        this.ctx.globalAlpha = letter.opacity * projected.scale; // Opacity giảm theo khoảng cách
        
        // Vẽ icon trước với scale
        const iconSize = letter.fontSize * 0.8 * projected.scale;
        this.ctx.font = `${iconSize}px Arial`;
        this.ctx.textAlign = 'left';
        this.ctx.textBaseline = 'middle';
        this.ctx.fillText(letter.icon, projected.x, projected.y);
        
        // Vẽ text với scale
        this.ctx.font = `500 ${letter.fontSize * projected.scale}px 'Open Sans', sans-serif`;
        this.ctx.fillText(letter.text, projected.x + iconSize + 10, projected.y);
    ...
    drawStar(star) {
        ...
        const brightness = star.brightness * twinkleBrightness;
        
        // Sao vẽ ở vị trí cố định (không bị ảnh hưởng bởi camera)
        this.ctx.globalAlpha = brightness;
        this.ctx.fillStyle = '#ffffff';
        this.ctx.shadowColor = '#ffffff';
        this.ctx.shadowBlur = 5 + star.size; // Hiệu ứng blur dựa theo kích thước sao
    ...
    animate() {
        // Tạo nền gradient vũ trụ với chiều sâu
        ...
        // Vẽ sao trước (background) - không bị ảnh hưởng bởi camera
        this.stars.forEach(star => {
            this.updateStar(star);
            this.drawStar(star);
        });

        // Cập nhật và vẽ tất cả chữ cái (foreground) - bị ảnh hưởng bởi camera
